...
carForm.addEventListener('submit', function (event) {
  ...
  if (editingId !== null) {
    ...
    if (index !== -1) {
      cars[index] = { ...carData, id: editingId }
    }
  } else {
    const newId = cars.length > 0 ? Math.max(...cars.map(c => c.id)) + 1 : 1
    cars.push({ ...carData, id: newId })
  }
...
function getCarFromForm() {
  ...
  if (!yearText || isNaN(year) || year < 1990 || year > maxYear) {
    alert(`연식은 1990년부터 ${maxYear}년(현재년도) 사이로 입력하세요.`)
    yearInput.focus()
    return null
  }

  if (!mileageText || isNaN(mileage) || mileage < 0) {
    alert('주행거리는 0이상 입력하세요.')
    mileageInput.focus()
    return null
  }

  if (!priceText || isNaN(price) || price < 1) {
    alert('가격은 1이상 입력하세요.')
    priceInput.focus()
    return null
  }
